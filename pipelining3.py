#!/usr/bin/env python3
"""
Fully Automated End-to-End Pipelining and Evaluation Script

Orchestrates the entire testing workflow:
1.  Prompts for account details for a multi-step run.
2.  Looks up tenantId from a central CSV file.
3.  Automatically generates the ground_truth.json for the tenant from instances.json.
4.  Uploads the relevant account structure file to a Google Cloud bucket.
5.  Fetches prediction data from the API and logs the request time.
6.  Saves results in a unique folder: outputs/<fileTypeId>/<tenantId>/
7.  Runs the full comparison and evaluation.
"""
...
def upload_account_structure_file(
    ...
    files = {'file': (destination_filename, open(source_file_path, 'rb'), 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')}
    ...
    try:
        ...
        print(f"   ✗ HTTP error during file upload: {http_err}"); print(f"   Response body: {response.text}"); return False
    except requests.exceptions.RequestException as req_err:
        print(f"   ✗ Request failed during file upload: {req_err}"); return False
    finally:
        if 'file' in files:
            files['file'][1].close()

# ... (Other helper functions like find_tenant_info, fetch_and_save_predictions, etc., are unchanged)
def find_tenant_info(tenant_id_to_find: str, tenant_info_folder: str) -> Optional[Dict[str, Any]]:
    ...
    return None
def fetch_and_save_predictions(api_endpoint: str, headers: Dict[str, str], payload: Dict[str, Any], output_dir: str, time_log_file: str) -> Optional[str]:
    ...
    try:
        ...
        log_prediction_time(time_log_file, payload.get('fileTypeId'), payload.get('tenantInformation', {}).get('globalTenantId'), duration)
        response.raise_for_status()
        ...
        print(f"   ✗ HTTP error: {http_err}"); print(f"   Response body: {response.text}"); return None
    except requests.exceptions.RequestException as req_err:
        print(f"   ✗ Request failed: {req_err}"); return None
    except json.JSONDecodeError:
        print(f"   ✗ Failed to decode JSON. Response text: {response.text}"); return None
...
class PredictionComparator:
    ...
    def _call_llm_for_match_analysis(self, field_data: Dict[str, Any]) -> str:
        ...
        prompt = """... (Full prompt is included) ..."""
        try:
...
def main():
    ...
    print("=" * 70)
    print("--- Automated Pipelining & Evaluation Workflow ---")
    print("=" * 70)

    # Collect Common Inputs
    file_type_id = input(f"Enter the fileTypeId to process [{DEFAULT_FILE_TYPE_ID}]: ") or DEFAULT_FILE_TYPE_ID
    num_accounts_str = input("How many account structures do you want to test for this file type? ")
    ...
    # The path for the instances file is the same for the whole batch
    ...
    if not os.path.exists(instances_json_path):
        print(f"✗ FATAL ERROR: The main instances file for this file type is missing.")
        print(f"  Please ensure this file exists: {instances_json_path}")
        return

    # Generate the field list once for the filetype
    ...
    # --- Main Loop for Each Account Structure ---
    for i in range(num_accounts):
        print("\n" + "=" * 70)
        print(f"Processing Account Structure {i+1}/{num_accounts}")
        print("=" * 70)

        account_structure_filename = input(f"[{i+1}] Enter the filename of the account structure (e.g., my_account.xlsx): ")
        integration_id = input(f"[{i+1}] Enter the corresponding integrationId for this account: ")

        # Step 1: Look up Tenant ID from CSV
        tenant_id = get_tenant_id_from_csv(ACCOUNTS_CSV_PATH, file_type_id, account_structure_filename)
        if not tenant_id:
            print(f"✗ Skipping this account: tenantId could not be found."); continue

        # Step 2: Define Paths and Auto-Generate Ground Truth File
        ...
        # Step 3: Upload Account Structure File
        source_file_to_upload = os.path.join(BASE_INSTANCES_FOLDER, file_type_id, account_structure_filename)
        file_extension = os.path.splitext(account_structure_filename)[1]
        ...
        # Step 4: Look up Tenant Info for Prediction Payload
        ...
        # Step 5: Construct Prediction API Payload and Headers
        ...
        # Step 6: Fetch Prediction from API
        ...
        # Step 7 (Optional): Fetch Prediction from Local Server
        if run_local_prediction == 'yes':
