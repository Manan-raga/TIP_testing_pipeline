...
2.  Looks up tenantId from a central CSV file (ignoring file extensions).
3.  Uploads the relevant account structure file with the required nomenclature.
4.  Fetches prediction data from the API and logs the request time.
5.  Optionally runs a prediction against a local server to capture logs.
6.  Saves results in a unique folder: outputs/<fileTypeId>/<tenantId>/
7.  Runs the full comparison and evaluation against a ground_truth.json.
"""
...
# CORRECTED: Fixed UUID for the upload filename prefix
UPLOAD_FILENAME_PREFIX_UUID = "ca28b853-27c3-433f-93eb-541f835269a6"


# --- Local File Paths (Now relative to the script's location for robustness) ---
...
def get_tenant_id_from_csv(csv_path: str, file_type_id: str, account_name: str) -> Optional[str]:
    ...
    # CORRECTED: Remove extension from the filename before searching
...
def upload_account_structure_file(
    ...
    """Uploads a file using a multipart/form-data request with a specific destination filename."""
...
# ... (All other helper functions and the PredictionComparator class are unchanged)
...
def main():
    ...
    for i in range(num_accounts):
        ...
        # --- Step 2: Upload Account Structure File with CORRECTED Nomenclature ---
        source_file_to_upload = os.path.join(BASE_INSTANCES_FOLDER, file_type_id, account_structure_filename)
        
        # CORRECTED: Construct the required destination filename using the FIXED UUID
        ...
        if not os.path.exists(ground_truth_file):
            print(f"✗ Ground truth file not found at '{ground_truth_file}'. Skipping evaluation."); continue
        ...
        if not exhaustive_field_list:
            print(f"✗ Could not generate field list. Skipping evaluation."); continue
